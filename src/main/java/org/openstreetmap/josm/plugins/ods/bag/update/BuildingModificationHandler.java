...
public class BuildingModificationHandler {
    ...
    private List<BuildingMapping> matches;

    public BuildingModificationHandler(OdsContext context) {
        super();
    ...
    private void analyseOdNodes() {
        ...
        odNodes.forEach((node, details) -> {
            ...
            if (osmNode != null && allowSnapping(osmNode)) {
    ...
    private static boolean allowSnapping(Node node) {
        for (var referrer : node.getReferrers()) {
            if (referrer.hasKey("building")) {
                return true;
            }
        }
        return false;
